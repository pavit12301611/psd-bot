...
This populates the `demo@psd_ai.local` Dovecot account (which has NO mbsync
...
USER = os.getenv("DEMO_IMAP_USER", "demo@psd_ai.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@psd_ai.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="psd_ai.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        ...
        text=("Hey,\n\nSaw your work on the psd.ai stack — seriously impressive. "
        ...
        html=("<p>Hey,</p><p>Saw your work on the <b>psd.ai</b> stack — seriously "
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@psd_ai-demo.example>",
    ...
    orig_id = make_msgid(domain="psd_ai.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@psd_ai-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — psd_ai-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: psd_ai-ui is returning 502s on the /api/chat endpoint.\n"
...
def _seed_cache() -> None:
    ...
    reply = (
        ...
        "— drafted for you by your psd.ai assistant"
    )
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@psd_ai-demo.example", summary, "demo", now))
        con.commit()
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    """
    safe_target = USER.endswith("@psd_ai.local") or HOST in ("localhost", "127.0.0.1", "::1")
...
def main() -> int:
    ...
    try:
        ...
        print(f"seeded {len(items)} demo message(s) into {USER} "
              f"(INBOX + Sent). Switch to the 'Demo' account in psd.ai to view.")
        return 0
    finally:
        try:
            conn.logout()
