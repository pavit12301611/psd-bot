"""gui/api_client.py — loopback HTTP client for the psd.ai desktop GUI.

The desktop application never re-implements any psd.ai business logic: it
talks to the exact same FastAPI server that the browser front-end uses.

* **attached** mode talks to ``http://127.0.0.1:<port>``.
* **embedded** mode drives the ``app`` object **in-process** through
  ``httpx.ASGITransport`` — no TCP server, no browser, no listening port.

All methods are async and are always awaited on the backend's dedicated event
loop (see :mod:`gui.backend`), never on Qt's loop, so the desktop stays
responsive and blocking server work happens inside the backend.

This is a thin wrapper over :mod:`httpx` — auth, sessions, history, model
endpoints, streaming and research all stay authoritative on the server.
"""
...
class GuiApiClient:
    ...
    def __init__(
        ...
        *,
        ...
        if cookie:
            self._client.cookies.set(SESSION_COOKIE, cookie)
    ...
    async def login(self, username: str, password: str, totp: str = "") -> Dict[str, Any]:
        ...
        if resp.status_code == 200 and party.get("ok"):
            session = ""
            try:
                session = str(resp.cookies.get(SESSION_COOKIE) or "")
            except Exception:
                session = ""
            if session:
                self._cookie = session
                self._client.cookies.set(SESSION_COOKIE, session)
            return {
                "ok": True,
                "username": str(party.get("username") or username),
                "session": session,
            }
        ...
        raise ApiError(
            f"login failed: {detail or f'HTTP {resp.status_code}'}",
            resp.status_code,
            detail,
        )

    async def setup_first_run(self, username: str, password: str) -> Dict[str, Any]:
        resp = await self._request(
            "POST", "/api/auth/setup", json_body={"username": username, "password": password}
        )
        party = resp.json() or {}
        if resp.status_code == 200 and party.get("ok"):
            return party
        raise ApiError(
            str(party.get("detail") or f"setup failed: HTTP {resp.status_code}"),
            resp.status_code,
            party.get("detail"),
        )
    ...
    async def rename_session(self, sid: str, name: str) -> None:
        self._ensure_2xx(
            await self._request("PATCH", f"/api/session/{sid}", data={"name": name})
        )
    ...
    async def history(self, sid: str, limit: int = 200) -> Dict[str, Any]:
        resp = self._ensure_2xx(
