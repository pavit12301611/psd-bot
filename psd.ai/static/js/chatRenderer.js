...
const _COST_LEDGER_LOCK = 'psd_ai-session-cost-ledger';
...
const _ACTION_RECENTS_KEY = 'psd_ai-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'psd_ai-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'psd.ai';
      ...
      row.addEventListener('click', () => {
        if (isToolApproval) {
          ...
          if (onSubmit) {
            ...
            card.remove();
            document.dispatchEvent(new CustomEvent('psd_ai:tool-approval', { detail }));
          }
        } else {
          send(label);
        }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'psd.ai' : modelRouteLabel(
