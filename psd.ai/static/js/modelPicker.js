...
const RECENT_KEY = 'psd_ai-model-recent';
const FAVORITES_KEY = 'psd_ai-model-favorites';
const RECENT_MAX = 5;
...
async function _ensureDefaultPendingChat() {
  ...
  try {
    let dc = null;
    try {
      dc = window.__psd_aiDefaultChat || null;
    } catch (_) {}
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__psd_aiChatBusy || Date.now() < (window.__psd_aiChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('psd_ai-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('psd_ai-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    _defaultPendingSeq++;
    try {
      window.__psd_aiLastPickedRoute = {
        model: m.mid || '',
        endpoint_url: m.url || '',
    ...
    try { window.__psd_aiModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__psd_aiModelSwitchPromise === switchPromise) delete window.__psd_aiModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('psd_ai:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('psd_ai:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__psd_aiDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('psd_ai-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `psd_ai-model-favorites[0]`
