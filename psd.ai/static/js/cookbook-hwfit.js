...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'psd.ai Cookbook hardware diagnostics',
      ...
      'docker compose exec psd_ai cat /proc/meminfo | head',
      'docker compose exec psd_ai python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed psd_ai. The user's selection must only change via
