...
document.addEventListener('psd_ai:tool-approval', () => {
...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.psd_ai-highlight').forEach(function(e) { e.classList.remove('psd_ai-highlight'); });
      document.querySelectorAll('.psd_ai-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('psd_ai-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'psd_ai-hl-label';
