...
export function wireArrowUpRecall(composer, getUserMessages, options = {}) {
  ...
  const debug = (...args) => {
    try {
      if (localStorage.getItem('psd_aiArrowRecallDebug') === '1') {
  ...
  composer.addEventListener('input', () => {
    ...
    lastRecalledValue = '';
    recallHistory = [];
    try { delete composer.dataset.psd_aiRecallIndex; } catch (_) {}
  });
  ...
  composer.addEventListener('keydown', (e) => {
    ...
    if (currentIndex < 0 && currentValue) {
      const markedIndex = Number(composer.dataset?.psd_aiRecallIndex);
    ...
    if (e.key === 'ArrowDown') {
      ...
      try { composer.dataset.psd_aiRecallIndex = String(nextIndex); } catch (_) {}
