...
def test_reference_discovery_covers_all_durable_upload_stores(
    ...
    try:
        ...
        db.add(Note(
            ...
            color=f"psd_ai://attachment/{note_color_id}",
        ))
        db.add(CalendarCal(
            id="calendar-1",
            owner="alice",
        ...
        db.add(CalendarEvent(
            ...
            description=f"Notes: psd_ai://attachment/{event_description_id}",
...
def test_write_reservation_extracts_only_explicit_internal_references():
    ...
    assert extract_internal_upload_ids({
        ...
        "nested": [f"psd_ai://attachment/{upload_id}"],
    }) == {upload_id}
    assert extract_internal_upload_ids(
    ...
    assert extract_internal_upload_ids(
        f"Attachment: psd_ai://attachment/{extensionless_id}: ready"
    ) == {extensionless_id}
    ...
    with pytest.raises(HTTPException) as calendar_error:
        asyncio.run(create_event(
            ...
            EventCreate(
                ...
                color=f"psd_ai://attachment/{upload_id}",
            ),
        ))
