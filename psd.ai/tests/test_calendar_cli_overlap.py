"""Regression: `psd_ai-calendar list` must select events that OVERLAP the
...
`psd_ai-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "psd_ai-calendar"
    loader = importlib.machinery.SourceFileLoader("psd_ai_calendar_cli", str(path))
