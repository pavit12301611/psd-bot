...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${PSD_AI_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${PSD_AI_OLLAMA_HOST}:${PSD_AI_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The PSD_AI_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `psd_ai-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("PSD_AI_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/psd_ai-test-content-cache")
...
def test_google_mcp_oauth_uses_configured_redirect_base(monkeypatch):
    monkeypatch.setenv("OAUTH_REDIRECT_BASE_URL", "https://psd_ai.example/app/")
    ...
    assert (
        ...
        == "https://psd_ai.example/app/api/mcp/oauth/callback"
    )
