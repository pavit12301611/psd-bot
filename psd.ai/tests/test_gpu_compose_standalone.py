...
base compose with only the matching overlay merged into the ``psd_ai``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on psd_ai only."""
...
# --- Non-psd_ai services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_psd_ai_services_match_base(base, standalone_path):
...
# --- psd_ai = base service + only the overlay additions ------------------


def test_nvidia_psd_ai_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_psd_ai_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
...
def test_base_has_no_host_docker_access(base):
    ...
    assert "PSD_AI_ENABLE_HOST_DOCKER=true" not in service["environment"]
...
def test_base_plus_host_docker_overlay_has_explicit_access(base):
    ...
    assert "PSD_AI_ENABLE_HOST_DOCKER=true" in service["environment"]
