...
def test_vllm_preflight_reports_cli_and_version():
    ...
    assert 'PSD_AI_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"' in script
    assert 'echo "[psd_ai] vLLM CLI: $PSD_AI_VLLM_BIN"' in script
    assert '"$PSD_AI_VLLM_BIN" --version' in script
    assert 'PSD_AI_PREFLIGHT_EXIT=127' in script
...
def test_serve_preflight_failure_keeps_tmux_pane_visible():
    ...
    assert 'PSD_AI_PREFLIGHT_EXIT=127' in script
    assert 'echo "=== Process exited with code $PSD_AI_PREFLIGHT_EXIT ==="' in script
...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "PSD_AI_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $PSD_AI_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$PSD_AI_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_psd_ai_has_cudart' in script
    ...
    assert script.index('_psd_ai_has_cudart') < script.index('DGGML_CUDA=ON')
...
def test_cached_model_scan_does_not_launch_ollama_cli_on_windows(tmp_path):
    ...
    env.pop("PSD_AI_ALLOW_OLLAMA_CLI_SCAN", None)
    proc = subprocess.run(
        [sys.executable, str(scan_py)],
        check=True,
        capture_output=True,
