#!/usr/bin/env python3
"""psd.ai — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("PSD_AI_ADMIN_USER", "").strip().lower()
        password = os.getenv("PSD_AI_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] PSD_AI_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] PSD_AI_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("PSD_AI_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("PSD_AI_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("PSD_AI_ADMIN_PASSWORD"):
...
def main():
    ...
    # Load .env so pre-seeded PSD_AI_ADMIN_USER / PSD_AI_ADMIN_PASSWORD (and
