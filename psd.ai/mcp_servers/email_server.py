...
_MCP_OWNER_ARG = "_psd_ai_owner"
_CURRENT_OWNER: ContextVar[str | None] = ContextVar("email_mcp_owner", default=None)
_OWNER_ENV_KEYS = ("PSD_AI_MCP_EMAIL_OWNER", "PSD_AI_EMAIL_OWNER")
_OWNER_SCOPE_ERROR = (
    "Error: email MCP requires an authenticated owner or PSD_AI_MCP_EMAIL_OWNER "
...
def _default_document_owner() -> str | None:
    ...
    """
    owner = os.environ.get("PSD_AI_DOCUMENT_OWNER", "").strip()
    if owner:
        return owner
    try:
...
def _fixture_account_rows() -> list[dict]:
    ...
    for row in _fixture_email_rows(owner or None):
        email_addr = row.get("account_email") or owner or "fixture@fixtures.psd_ai.local"
        if email_addr not in owners:
            owners.append(email_addr)
    if not owners:
        owners = [owner or "fixture@fixtures.psd_ai.local"]
    return [
        {
            "id": "fixture-email",
    ...
    try:
        ...
        conn.execute("""
            ...
            (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr,
             attachments, send_at, created_at, status, account_id, psd_ai_kind, owner)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'agent_draft', ?, ?, ?)
        """, (
            pending_id,
...
def _create_email_draft_document(
    *,
    ...
    """Create an psd.ai email compose document for user review. Does not send."""
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded psd.ai reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with psd.ai' AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "List the email accounts configured in psd.ai. Returns each account's "
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email so the user can review and send from psd.ai. "
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new psd.ai email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in psd.ai. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "cc": {"type": "string", "description": "CC address(es), comma-separated (optional)"},
                    "bcc": {"type": "string", "description": "BCC address(es), comma-separated (optional)"},
                    "title": {"type": "string", "description": "Optional psd.ai document title"},
                    **ACCOUNT_PROP,
                },
                "required": ["to", "subject", "body"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email_reply so the user can review and send from psd.ai. "
        ...
        Tool(
            ...
            description=(
                "Create an psd.ai email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using psd.ai' existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in psd.ai before delivery."
                    ),
                )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created psd.ai email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in psd.ai to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created psd.ai reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in psd.ai to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created psd.ai compose draft "
