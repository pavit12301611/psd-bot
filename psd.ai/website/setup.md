...
> **Branch note:** `dev` is the default branch and contains the latest development changes, but it may be unstable. For the more stable curated branch, use [`main`](https://github.com/pavit12301611/psd-bot/tree/main).

Defaults work out of the box: clone, run, then configure models/search/email
inside **Settings**. Only edit `.env` for deployment-level overrides like
`APP_BIND`, `APP_PORT`, `AUTH_ENABLED`, `DATABASE_URL`, or a pre-seeded admin password.

On first setup, psd.ai creates an admin account (`admin` unless
`PSD_AI_ADMIN_USER` is set) and prints a temporary password in the terminal.
For Docker installs, the same line is in `docker compose logs psd_ai`.
Use that for the first login, then change it in **Settings**.

Contributing? See [CONTRIBUTING.md](https://github.com/pavit12301611/psd-bot/blob/dev/CONTRIBUTING.md) for setup, testing, and pull request guidelines.
...
**Docker bundled services.** Compose starts psd.ai, ChromaDB, SearXNG, and
ntfy. psd.ai and the bundled service ports bind to `127.0.0.1` by default, so
...
psd.ai SSH key and add the public key to the remote server's
...
does not mount `/var/run/docker.sock`. You can still connect psd.ai to
...
On Arch Linux, verify the host NVIDIA driver and Docker GPU passthrough before enabling the psd.ai NVIDIA overlay.
...
Then enable the psd.ai NVIDIA compose overlay:

```env
COMPOSE_FILE=docker-compose.yml:docker/gpu.nvidia.yml
```

Rebuild and verify the GPU inside the psd.ai container:

```bash
docker compose up -d --build
docker compose exec psd_ai nvidia-smi -L
```
...
```bash
docker compose exec psd_ai nvidia-smi -L   # NVIDIA
docker compose exec psd_ai sh -lc 'test -e /dev/kfd && test -d /dev/dri && ls -l /dev/kfd /dev/dri/renderD*'  # AMD
```
...
This connects psd.ai in Docker to an Ollama server that is already running on
...
If `chromadb-client` (the lightweight HTTP-only package) is installed alongside the full `chromadb` package, psd.ai starts but ChromaDB silently falls back to HTTP-only mode and fails.
...
To expose psd.ai on a local network or Tailscale with HTTPS:
1. Change the bind address to `0.0.0.0` in `.env` (`APP_BIND=0.0.0.0` or `PSD_AI_HOST=0.0.0.0`).
...
3. Run `uvicorn` with the generated certs:
   ...
   ```
4. Install the `mkcert` CA on any other device you want to access psd.ai from (e.g., for iOS, email the `rootCA.pem` to yourself, install the profile, and trust it in Certificate Trust Settings).
...
- **Calendar/contacts (Radicale) won't sync.** Point psd.ai at the **full collection URL** with its trailing slash — e.g. `http://host:5232/<user>/<collection-id>/` — not just the server root. Radicale shows this address for each calendar/address book in its web UI.
...
| `kokoro`, `soundfile` | Local Kokoro-82M text-to-speech on a CUDA GPU. The pinned Kokoro release supports psd.ai installs on Python 3.11-3.12; these packages are intentionally skipped on Python 3.13+ (including the Python 3.14 container image). |
...
The default Docker image currently uses Python 3.14, while Kokoro 0.9.4 declares Python `>=3.10,<3.13`. psd.ai itself continues to support Python 3.11+, but this pinned optional local-TTS feature requires a native Python 3.11 or 3.12 environment. Kokoro declares `torch`, but the local provider only activates when that torch build has CUDA and a GPU is visible; install the CUDA build appropriate for your host. Browser and configured endpoint TTS remain available on Python 3.13+ and in the container image.
...
- Keep ChromaDB, SearXNG, ntfy, Ollama, vLLM, llama.cpp, databases, and raw model/provider APIs internal-only. Expose only the authenticated psd.ai web/API entrypoint through your trusted proxy or private access layer.
...
psd.ai serves plain HTTP on its app port. Docker Compose binds psd.ai and the bundled services to `127.0.0.1` by default, so a typical production/private setup is:

1. Keep psd.ai on localhost, for example `127.0.0.1:7000`.
2. Terminate HTTPS at a trusted reverse proxy or private access gateway.
3. Put the authenticated psd.ai web/API entrypoint behind that layer.
4. Keep raw service and model ports internal-only.

Cloudflare Access, Tailscale, Caddy, nginx, and Traefik can all fit this pattern; none are required by psd.ai. If your access layer reaches psd.ai on the same host, proxy to `http://127.0.0.1:7000` and keep `AUTH_ENABLED=true` and `LOCALHOST_BYPASS=false`. Any proxy that forwards `X-Forwarded-Proto: https` gets `Secure` session cookies without configuration, so `SECURE_COOKIES` only needs setting when you want to override that — force it on for a proxy that forwards no scheme at all, or off while you still serve plain HTTP.
...
psd.ai needs no changes for this — uvicorn keeps speaking HTTP/1.1 on
...
(`psd.ai.example.com:8443`) and use it in the URL. That alone does not free
...
**4. Point psd.ai at the new origin** in `.env`, then restart it.

A proxy that exposes the HTTPS request scheme to psd.ai needs no `SECURE_COOKIES` setting. Only force it on when the proxy cannot expose that scheme:

```bash
# only if the proxy cannot expose the external HTTPS scheme to psd.ai:
SECURE_COOKIES=true
# only if you use remote MCP servers with OAuth:
OAUTH_REDIRECT_BASE_URL=https://psd.ai.example.com
```
...
```bash
GOOGLE_OAUTH_REDIRECT_URI=https://psd.ai.example.com/api/email/oauth/google/callback
```
...
- Leave `SECURE_COOKIES` unset when psd.ai can see the external HTTPS scheme;
...
- psd.ai sends `Strict-Transport-Security` once it sees `X-Forwarded-Proto:
  ...
  have visited psd.ai. Give psd.ai its own hostname, or strip the header at
...
| `PSD_AI_CHAT_UPLOAD_MAX_BYTES` | `10485760` | Chat/agent attachment cap in bytes. Raise for larger local PDFs or text documents. |
| `PSD_AI_GALLERY_UPLOAD_MAX_BYTES` | `104857600` | Gallery image upload cap in bytes (100 MB). |
| `PSD_AI_GALLERY_TRANSFORM_UPLOAD_MAX_BYTES` | `26214400` | Gallery transform input cap in bytes (25 MB). |
| `PSD_AI_MEMORY_IMPORT_MAX_BYTES` | `10485760` | Memory import file cap in bytes (10 MB). |
| `PSD_AI_PERSONAL_UPLOAD_MAX_BYTES` | `26214400` | Personal document upload cap in bytes (25 MB). |
| `PSD_AI_EMAIL_COMPOSE_UPLOAD_MAX_BYTES` | `26214400` | Email compose attachment cap in bytes (25 MB). |
| `PSD_AI_STT_MAX_AUDIO_BYTES` | `26214400` | Speech-to-text audio cap in bytes (25 MB). |
| `PSD_AI_ICS_MAX_BYTES` | `10485760` | Calendar `.ics` import cap in bytes (10 MB). |

All upload-limit vars are validated (must be a positive integer) and optional; an invalid value fails fast at startup.

### Built-in MCP servers (optional setup)

psd.ai auto-registers a few built-in MCP servers at startup. The npx-based ones (currently the browser server, `@playwright/mcp`) only start when their npm package is already in the local npx cache. If a package isn't cached, that server is skipped with a startup log message explaining what to do, so a fresh install does not block on a multi-minute npm download or hang if Playwright system deps are missing.
...
That installs `@playwright/mcp` plus Playwright (~300MB total). Restart psd.ai and the server will register at startup.
