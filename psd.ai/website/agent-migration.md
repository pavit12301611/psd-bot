...
psd.ai should be able to learn from another agent without blindly trusting
...
Markdown notes, or any other agent can have its own adapter, but psd.ai only
...
- **Memory candidates** are short facts or preferences that can be reviewed
  before being saved into psd.ai memory.

This keeps psd.ai' existing memory-review flow intact while giving it better
...
The helper does not write to `data/`, call an LLM, import psd.ai modules, or
...
A future psd.ai importer should treat the manifest as untrusted user-provided
...
Nonstandard folders should be adapter details, not required psd.ai concepts.
