                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in psd.ai (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in psd.ai" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "psd_ai_kind" if "psd_ai_kind" in cols else "'scheduled' AS psd_ai_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `psd_ai-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the PSD_AI_INPROCESS_POLLERS=0 guidance) -
                ...
                psd_ai_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-psd.ai-Origin"] = "psd_ai-ui"
                outer["X-psd.ai-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", psd_ai_kind or "scheduled")[:64]
                outer["X-psd.ai-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `psd_ai-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `PSD_AI_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `psd_ai-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("PSD_AI_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `PSD_AI_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (PSD_AI_INPROCESS_POLLERS=0); "
            "drive `psd_ai-mail poll-scheduled` externally."
        )
        return
    import asyncio
