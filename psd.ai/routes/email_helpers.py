...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. psd.ai "
...
# Override at deploy time via PSD_AI_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "psd_ai_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN psd_ai_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("PSD_AI_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for psd.ai-generated mail (e.g. reminder, scheduled).
    psd_ai_kind: Optional[str] = None
