...
def _parse_list_unsubscribe_header(value: str | None) -> list[dict]:
    ...
    first-pass psd.ai flow. HTTP unsubscribe links are useful evidence but
...
def _apply_psd_ai_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-psd.ai-Origin"] = PSD_AI_MAIL_ORIGIN
    if kind:
        msg["X-psd.ai-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-psd.ai-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.psd_ai.local>",
            "subject": subject,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the psd.ai marker header, but include the subject
                # fallback too. The fallback uses a distinct psd.ai prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-psd.ai-Kind "reminder" SUBJECT "Reminder (psd.ai):"{from_clause})',
                )
    ...
    def execute_unsubscribe(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            msg_out["Message-ID"] = email.utils.make_msgid(domain="psd_ai.local")
            _apply_psd_ai_headers(msg_out, "unsubscribe", uid)
    ...
    @router.delete("/psd_ai/reminders")
    async def delete_psd_ai_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as psd.ai reminders."""
        if account_id:
            _assert_owns_account(account_id, owner)
        deleted = 0
        folders_checked = []
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-psd.ai-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (psd.ai):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (psd.ai):")})'))
                            # Legacy reminders created before the psd.ai
            ...
            logger.error(f"delete_psd_ai_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    def _load_psd_ai_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-psd_ai")
    async def compose_from_psd_ai(data: dict, owner: str = Depends(require_owner)):
        """Stage an psd.ai document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_psd_ai_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage psd.ai attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-psd_ai-zip")
    async def compose_from_psd_ai_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several psd.ai documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("psd_ai-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage psd.ai zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", psd_ai_kind=None, psd_ai_ref=None,
    ):
        ...
        _apply_psd_ai_headers(outer, psd_ai_kind or "scheduled", psd_ai_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, psd_ai_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("psd_ai_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="psd_ai.local")
        ...
        if req.psd_ai_kind:
            _apply_psd_ai_headers(outer, req.psd_ai_kind)
