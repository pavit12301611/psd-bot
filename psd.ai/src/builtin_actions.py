...
async def action_ssh_command(owner: str, command: str = "", host: str = "localhost", **kwargs) -> Tuple[str, bool]:
    ...
    if host in ("localhost", "127.0.0.1", "local"):
        # Explicit bash, not shell=True: subprocess would hand the command to
        # /bin/sh (dash on some spins), which rejects the bash syntax the agent
...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("PSD_AI_SCRIPT_HOST", "localhost")).strip()
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                ...
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip psd.ai-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get("X-psd.ai-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-psd.ai-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "psd_ai-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (psd_ai):")
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on PSD_AI_SCRIPT_HOST",
