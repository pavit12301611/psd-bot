...
def _find_npx() -> str:
    """Find the npx binary, checking the usual per-user locations if it is not
    on PATH.

    A server process inherits a minimal environment, so an nvm- or
    npm-prefix-installed Node is invisible here even though the user's shell
    finds it. Those locations are probed explicitly rather than failing.
    """
    npx = which_tool("npx")
    if npx:
        return npx
    ...
    if node:
        cand = os.path.join(os.path.dirname(node), "npx")
...
MCP_DISABLED = os.environ.get("PSD_AI_DISABLE_MCP", "").lower() in ("1", "true", "yes")
BROWSER_MCP_REQUIRE_CACHE = os.environ.get("PSD_AI_BROWSER_MCP_REQUIRE_CACHE", "").lower() in ("1", "true", "yes")
...
def _find_browser_executable() -> str:
    ...
    """
    configured = os.environ.get("PSD_AI_BROWSER_EXECUTABLE", "").strip()
    if configured:
        return configured
...
def _browser_mcp_args(args: list[str]) -> list[str]:
    ...
    if os.environ.get("PSD_AI_BROWSER_ISOLATED", "1").lower() not in ("0", "false", "no"):
        if "--isolated" not in out and "--user-data-dir" not in out:
            out.append("--isolated")
    if os.environ.get("PSD_AI_BROWSER_NO_SANDBOX", "1").lower() not in ("0", "false", "no"):
...
async def register_builtin_servers(mcp_manager):
    ...
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via PSD_AI_DISABLE_MCP")
        return
    ...
    async def _start_npx_servers():
        ...
        for server_id, cfg in _BUILTIN_NPX_SERVERS.items():
            ...
            # with PSD_AI_BROWSER_MCP_REQUIRE_CACHE=1.
            ...
            if BROWSER_MCP_REQUIRE_CACHE and pkg_spec and not await _is_npx_package_cached(npx_path, pkg_spec):
                logger.warning(
                    ...
                    f"          (run once, then restart psd.ai)\n"
                    f"  Notes:  PSD_AI_BROWSER_MCP_REQUIRE_CACHE=1 is set, "
                    f"so psd.ai will not install browser automation on startup."
                )
                continue
