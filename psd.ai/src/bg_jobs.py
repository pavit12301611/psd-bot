...
def _pid_alive(pid: Optional[int]) -> bool:
    # Delegates to the shared probe, which treats EPERM as "alive but owned by
    # someone else" instead of reporting the job as gone.
...
def launch(command: str, session_id: str, cwd: Optional[str] = None,
    ...
    # exit status.
    # The user command goes in its OWN script file, run as a child shell — an
    # `exit` inside it only ends that child (so the wrapper still records the
    # exit code), and an unbalanced paren or trailing line-continuation in the
    # command can't break the wrapper. `$?` is the child's real exit status.
    # Paths are shell-quoted so a job directory with spaces still works.
    shell = find_bash() or "sh"
    cmd_path = _JOBS_DIR / f"{job_id}.cmd.sh"
    cmd_path.write_text(command + "\n", encoding="utf-8")
    lp, xp, cp = (shlex.quote(posix_path(p)) for p in (log_path, exit_path, cmd_path))
    ...
    proc = subprocess.Popen(
        ...
        cwd=cwd or None,
        **detached_popen_kwargs(),  # setsid: detach from the request lifecycle
    )
...
def _kill(pid: Optional[int]) -> None:
    # Signals the whole process group, so a job that spawned children (a build,
    # a model download) cannot leave orphans holding a GPU or a port.
