...
COLLECTION_NAME = "psd_ai_tool_index"
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "computer_control": "Control the user's real desktop: click and move the mouse, type text, press keyboard shortcuts, open applications/files/websites, focus or close windows, read and write the clipboard, adjust system volume, list or terminate processes, send a notification. Use it when the user asks you to do something ON THEIR PC ('open firefox', 'turn the volume down', 'press ctrl+s', 'close that window', 'what's on my screen'). Never invent screen coordinates: call computer_screen or the windows action first. Do not use bash for desktop automation when this tool fits.",
    "computer_screen": "Take a screenshot of the user's screen to see what is currently displayed (primary monitor by default). Use it before clicking or typing when you do not know where a control is, or when the user asks 'what's on my screen'. The image is captured for the user; describe what they should look at rather than claiming to see pixels.",
