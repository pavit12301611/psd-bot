...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "computer_control": "Control the user's real desktop: click and move the mouse, type text, press keyboard shortcuts, open applications/files/websites, focus or close windows, read and write the clipboard, adjust system volume, list or terminate processes, send a notification. Use it when the user asks you to do something ON THEIR PC ('open notepad', 'turn the volume down', 'press ctrl+s', 'close that window', 'what's on my screen'). Never invent screen coordinates: call computer_screen or the windows action first. Do not use bash for desktop automation when this tool fits.",
