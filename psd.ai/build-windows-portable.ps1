...
<#
  Build a portable Windows distribution for psd.ai.

  Output layout:
    dist\psd.ai\psd.ai.exe
    dist\psd.ai\static\...
    dist\psd.ai\scripts\...
    dist\psd.ai\mcp_servers\...
    dist\psd.ai\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name psd.ai @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\psd.ai" -ForegroundColor Green
